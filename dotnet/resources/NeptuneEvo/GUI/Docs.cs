    ...
    class Docs : Script
    {
        ...
        public static void DocsFunction(ExtPlayer player, ExtPlayer target, int docsEnum)
        {
            try
            {
                ...
                //Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouOfferShowDoc, DocsArray[docsEnum].SendFrom), 5000);
               // Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PlayerOfferShowDoc, player.Value, DocsArray[docsEnum].Name), 6000);
                EventSys.SendCoolMsg(player,"Предложение", "Документы", $"{LangFunc.GetText(LangType.En, DataName.YouOfferShowDoc, DocsArray[docsEnum].SendFrom)}", "", 7000);
                EventSys.SendCoolMsg(target,"Предложение", "Документы", $"{LangFunc.GetText(LangType.En, DataName.PlayerOfferShowDoc, player.Value, DocsArray[docsEnum].Name)}", "", 8000);
                string genderWord = characterData.Gender == false ? LangFunc.GetText(LangType.En, DataName.dostal) : LangFunc.GetText(LangType.En, DataName.dostala);
        ...
        public static void Event_Certificate(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.OnlyFracCan), 3000);
                    return;
                }
                if (!Fractions.Manager.GovIds.Contains(memberFractionData.Id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.OnlyGosFracCan), 3000);
                    return;
                }
        ...
        public static void AcceptPasport(ExtPlayer player)
        {
            try
            {
                ...
                string gender = (fromCharacterData.Gender) ? LangFunc.GetText(LangType.En, DataName.Mans) : LangFunc.GetText(LangType.En, DataName.Womens);
                string fraction = (fromMemberFractionData != null) ? Fractions.Manager.FractionNames[fromMemberFractionData.Id] : LangFunc.GetText(LangType.En, DataName.No);
                if (fromMemberFractionData != null && ((fromMemberFractionData.Id >= (int)Fractions.Models.Fractions.FAMILY && fromMemberFractionData.Id <= (int)Fractions.Models.Fractions.BLOOD) || fromMemberFractionData.Id == (int)Fractions.Models.Fractions.FIB || (fromMemberFractionData.Id >= (int)Fractions.Models.Fractions.LCN && fromMemberFractionData.Id <= (int)Fractions.Models.Fractions.ARMENIAN))) 
                    fraction = LangFunc.GetText(LangType.En, DataName.No);
                string work = (fromCharacterData.WorkID > 0) ? Jobs.WorkManager.JobStats[fromCharacterData.WorkID - 1] : LangFunc.GetText(LangType.En, DataName.NoWorker);
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PlayerShowYouPass, from.Value), 5000);
                Notify.Send(from, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouShowPassPlayer, player.Value), 5000);
                if (fromCharacterData.Gender) Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.HeShow, player.Name), player);
                else Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.SheShow, player.Name), player);
        ...
        public static void AcceptLicenses(ExtPlayer player)
        {
            try
            {
                ...
                if (lic == "") lic = LangFunc.GetText(LangType.En, DataName.None);
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PlayerShowLicyou, from.Value), 5000);
                Notify.Send(from, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouShowLicPlayer, player.Value), 5000);
                if (fromCharacterData.Gender) Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.HeShowLic, player.Name), player);
                else Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.SheShowLic, player.Name), player);
                string json = Newtonsoft.Json.JsonConvert.SerializeObject(data);
        ...
        public static void AcceptIdcard(ExtPlayer player)
        {
            try
            {
                ...
                Dictionary<string, string> RequestData = new Dictionary<string, string>
                {
                    ...
                    ["gender"] = (fromCharacterData.Gender) ? LangFunc.GetText(LangType.En, DataName.Mans) : LangFunc.GetText(LangType.En, DataName.Womens),
                    ["lic"] = lic
                };

                string json = Newtonsoft.Json.JsonConvert.SerializeObject(RequestData);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PlayerShowIdToYou, from.Value), 5000);
                Notify.Send(from, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouShowIdToPlayer, player.Value), 5000);
                if (fromCharacterData.Gender) Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.HePokazal, player.Value), player);
                else Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.ShePokazala, player.Value), player);
        ...
        public static void AcceptCertificate(ExtPlayer player)
        {
            try
            {
                ...
                string json = Newtonsoft.Json.JsonConvert.SerializeObject(RequestData);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PokazalVamUdostoverenie, from.Value), 5000);
                Notify.Send(from, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouPokazalUdostoverenie, player.Value), 5000);
                if (fromCharacterData.Gender) Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.PokazalUdo, player.Value), player);
                else Commands.RPChat("sme", from, LangFunc.GetText(LangType.En, DataName.PokazalaUdo, player.Value), player);
            ...
            try
            {
                ...
                switch (action)
                {
                    case "Посмотреть значок":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PlayerNoBadge), 3000);
                            return;
                        }

                        if (characterData.Gender) Commands.RPChat("sme", player, LangFunc.GetText(LangType.En, DataName.HeSeeZnak, target.Name), target);
                        else Commands.RPChat("sme", player, LangFunc.GetText(LangType.En, DataName.SheSeeZnak, target.Name), target);
                    ...
                    case "Посмотреть бейджик":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.PlayerNoBadges), 3000);
                            return;
                        }

                        if (characterData.Gender) Commands.RPChat("sme", player, LangFunc.GetText(LangType.En, DataName.HeSeeBadge, target.Name), target);
                        else Commands.RPChat("sme", player, LangFunc.GetText(LangType.En, DataName.SheSeeBadge, target.Name), target);
