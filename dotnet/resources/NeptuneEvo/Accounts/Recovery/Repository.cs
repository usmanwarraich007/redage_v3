    ...
    class Repository
    {
        ...
        public static void SendEmail(ExtPlayer player, string login)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.RecoveryCantFind), 4500);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.RecoveryEmailCant), 4500);
                    Trigger.ClientEvent(player, "restorepassstep", 2);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.RecoveryError), 5000);
                    Trigger.ClientEvent(player, "restorepassstep", 2);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"The message with the password recovery code has been successfully sent to {Generate.ObfuscateEmail(auntificationData.Email)}.", 5000);
        ...
        public static async void RecoveryPassword(ExtPlayer player, string code)
        {
            try
            {
                ...
                {
                    Log.Debug($"{sessionData.RealSocialClub} Password successfully recovered!", nLog.Type.Info);
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You have successfully restored access to your account. A new password has been sent to your email!", 5000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Additionally, you can change it right now in-game by typing /password [new password]. Example: /password 123", 10000);
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.CodeDoesntMatter), 4500);
                    Trigger.ClientEvent(player, "restorepassstep", 1);
                }
            }
            catch (Exception e)
