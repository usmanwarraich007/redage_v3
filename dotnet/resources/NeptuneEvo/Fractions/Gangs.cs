    ...
    class Gangs : Script
    {
        ...
        public void Event_OnResourceStart()
        {
            try
            {
                ...
                BuyDruygsLab[0] = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(LangFunc.GetText(LangType.En, DataName.TextLabelDrugs, Main.DrugsPrice), DrugPoints[0] + new Vector3(0, 0, 0.7), 5f, 0.3f, 0, new Color(255, 255, 255), true, 0);
                BuyDruygsLab[1] = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(LangFunc.GetText(LangType.En, DataName.TextLabelDrugs, Main.DrugsPrice), DrugPoints[1] + new Vector3(0, 0, 0.7), 5f, 0.3f, 0, new Color(255, 255, 255), true, 0);
        ...
        public static void OnDrugPoints(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouMustInNarcoCar), 3000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouCantBuyNarco), 3000);
                        return;
                    }
                    if (!player.IsFractionAccess(RankToAccess.BuyDrugs)) return;
                    Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.En, DataName.BuyDrugs), LangFunc.GetText(LangType.En, DataName.HowMuch), 4, "buy_drugs");
                }
            }
            catch (Exception e)
            {
        ...
        public static void BuyDrugs(ExtPlayer player, int amount)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.NotEnoughWarehouseGangMoney), 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.CarDrugsHowMuchCan, vMain.GetMaxSlots(vehicle.Model) * Chars.Repository.ItemsInfo[ItemId.Drugs].Stack), 8000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.En, DataName.YouBoughtNarcoGrams, amount), 5000);
                    Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeMoney, LangFunc.GetText(LangType.En, DataName.BuyNarco, amount));
                }
            }
            catch (Exception e)
            {
